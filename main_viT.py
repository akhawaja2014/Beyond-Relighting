...
class FolderImageSimilarityCalculator:
    def __init__(self, model_name='resnet50'):
        ...
        # Load weights + correct transforms when available
        try:
            weights_enum = getattr(models, f"{self.model_name.upper()}_Weights")
            weights = weights_enum.DEFAULT
            backbone = getattr(models, self.model_name)(weights=weights)
        ...
        # Detect torchvision ViT by its components (forward_features may not exist in your version)
        self.is_vit = all([
            hasattr(backbone, "conv_proj"),
            hasattr(backbone, "encoder"),
            hasattr(backbone, "class_token"),
        ])

        if self.is_vit:
            # Keep full ViT; we will manually run tokens -> encoder -> CLS
            self.vit = backbone.to(self.device).eval()
            for p in self.vit.parameters():
                p.requires_grad = False
            self.backbone = None  # not used
        else:
            # ResNet path: strip classifier
            self.backbone = nn.Sequential(*list(backbone.children())[:-1]).to(self.device).eval()
            for p in self.backbone.parameters():
                p.requires_grad = False
            self.vit = None

    @torch.inference_mode()
    def _vit_forward_features(self, x):
        """
        Torchvision ViT feature extraction (CLS embedding):
        x: (B, 3, H, W) -> tokens -> prepend CLS -> add pos emb -> encoder -> take CLS.
        Returns (B, D), L2-normalized.
        """
        vit = self.vit
        # 1) Patchify to tokens (B, N, D)
        if hasattr(vit, "_process_input"):
            tokens = vit._process_input(x)                     # (B, N, D)
        else:
            # Fallback: conv_proj -> flatten -> transpose
            y = vit.conv_proj(x)                               # (B, D, H', W')
        ...
        # 3) Add positional embeddings (assume shape matches)
        if hasattr(vit.encoder, "pos_embedding"):
            z = z + vit.encoder.pos_embedding                  # (B, 1+N, D)

        # 4) Dropout + layers + LayerNorm
        if hasattr(vit.encoder, "dropout"):
            z = vit.encoder.dropout(z)
        if hasattr(vit.encoder, "layers"):
            z = vit.encoder.layers(z)                          # transformer blocks
    ...
    def _forward_features(self, x):
        ...
        """
        if self.is_vit:
            return self._vit_forward_features(x)
        else:
            feats = self.backbone(x).flatten(1)                # (B, D) for ResNet
            feats = F.normalize(feats, p=2, dim=1)
            return feats
    ...
    def extract_features(self, image):
        # image: (1, C, H, W)
        feats = self._forward_features(image.to(self.device, non_blocking=True))  # (1, D)
...
def main():
    ...
    # RTI_sim = FolderImageSimilarityCalculator(model_name='resnet50')
    # RGB_sim = FolderImageSimilarityCalculator(model_name='resnet50')

    RTI_sim = FolderImageSimilarityCalculator(model_name='vit_l_16')
    RGB_sim = FolderImageSimilarityCalculator(model_name='vit_l_16')
    ...
    best_result_RTI = RTI_sim.cluster_and_evaluate(RTI_features_list, max_k=10,dataset_name ="RTI", plot_path="figures/ViTb16/figuresRTI")
    RTI_cluster_labels = best_result_RTI['labels']

    best_result_RGB = RGB_sim.cluster_and_evaluate(RGB_features_list, max_k=10, dataset_name ="RGB", plot_path="figures/ViTb16/figuresRGB")
    ...
    with open("cluster_assignments_VIT_RTI_VP_PCA4Kmeans.csv", "w", newline='') as f:
        writer = csv.writer(f)
    ...
    with open("cluster_assignments_VIT_RGB_VP_PCA4Kmeans.csv", "w", newline='') as f:
        writer = csv.writer(f)
    ...
    RTI_sim.visualize_tsne(RTI_features_list, image_paths_RTI, output_path='figures/ViTb16/figuresRTI/RTIVP_tsne.pdf', perplexity=5)
    RGB_sim.visualize_tsne(RGB_features_list, image_paths_RGB, output_path='figures/ViTb16/figuresRGB/RGBVP_tsne.pdf', perplexity=5)

    print("Thank you for your patience")
    RTI_sim.visualize_umap(RTI_features_list, image_paths_RTI, output_path='figures/ViTb16/figuresRTI/rtiVP_umap.pdf', n_neighbors=5, min_dist=0.5)
    RGB_sim.visualize_umap(RGB_features_list, image_paths_RGB, output_path='figures/ViTb16/figuresRGB/rgbVP_umap.pdf', n_neighbors=5, min_dist=0.5)   
    ...
    RTI_sim.visualize_feature_space(RTI_features_list, image_paths_RTI, output_path='figures/ViTb16/figuresRTI/RTIVP_pca_2d.pdf')
    RGB_sim.visualize_feature_space(RGB_features_list, image_paths_RGB, output_path='figures/ViTb16/figuresRGB/RGBVP_pca_2d.pdf')
    # Calculate and visualize MDS
    distance_matrix, _ = RGB_sim.calculate_distance_matrix(processed_images_RGB)

    # Visualize MDS in 2D
    RTI_sim.visualize_mds(distance_matrix, RTI_features_list, image_paths_RTI, n_components=2,  output_path='figures/ViTb16/figuresRTI/RTIVP_euc_mds_2d.pdf')
    RGB_sim.visualize_mds(distance_matrix,  RGB_features_list, image_paths_RGB, n_components=2,  output_path='figures/ViTb16/figuresRGB/RGBVP_euc_mds_2d.pdf')
...
if __name__ == "__main__":
    main()
    # --- The rest of your methods (similarity/visualizations/clustering) stay unchanged ---